...
export const DEPARTMENT_OPTIONS = Array.from(new Set(RAW_DEPARTMENTS)).sort((a, b) =>
...
export const YEAR_OPTIONS = Array.from({ length: YEAR_RANGE }, (_, index) => CURRENT_YEAR - index);
...
export const LEVEL_LABELS = LEVEL_OPTIONS.reduce<Record<number, string>>((acc, option) => {
...
export const YEARS_MENU: Menu = {
  年度を選択: Array.from({ length: YEAR_RANGE }, (_, index) => `${CURRENT_YEAR - index}年度`)
};
