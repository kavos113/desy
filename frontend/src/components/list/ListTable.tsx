...
const ListTable = ({ items, onSort, className }: ListTableProps) => {
  ...
  const prefetchRelatedLectures = useCallback(
    async (lecture: domain.Lecture) => {
      ...
      const targets = relatedIds.filter(
        (relatedId) => !baseCache[relatedId] && relatedId !== lecture.ID
      );
      if (targets.length === 0) {
        return;
      }
    ...
    <div className={wrapperClassName}>
      <div className="list-table">
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => <ListItem key={item.ID} item={item} onClick={handleListItemClick} />)
        )}
